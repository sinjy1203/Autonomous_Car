...
class Cal:
    def __init__(self, frame_shape=(640, 480), param_dir='./camera_parameter',
                 param_name='cam_calib.pkl',
                 checkerboards_dir='./data/raw_chess',
    ...
    def camera_calibration(self, corner_count=(8, 6), show_corner=True,
                           param_name='cam_calib.pkl'):
...
if __name__ == "__main__":
    # cal = Cal(predict=False)
    # # cal.undistort_webcam()
    # cal.camera_calibration()
    f = open('./camera_parameter/cam_calib.pkl', 'rb')
    mat, dist, rvecs, tvecs = pickle.load(f)
    np.save('./camera_parameter/cm', mat)
    np.save('./camera_parameter/d', dist)
    print(mat)
    print(dist)
